...
class Cora_Dataset(Dataset):
     ...
     def initialize_sns(self):
        ...
        for node in range(len(self.node_feature)):
            ...
            negative_pairs = [(node, neg) for neg in random.sample(range(len(self.node_feature)), 
                            len(positive_pairs)) if neg not in self.train_L1[node]]
