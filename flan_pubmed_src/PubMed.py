        ...
        for key in list(self.task_list.keys()):     
            if key == 'link':
                for tems in self.task_list[key]: 
                    if '1-3-1-1' in tems:
                        self.total_length += 19717 * 1  # 19717 nodes in PubMed Graph
                ...
                for tems in self.task_list[key]:
                    if '2-3-1-2' in tems:
                        self.total_length += len(self.classification) * 2  # suggest times 20 for small-pubmed in one epoch   
    ...
    def __getitem__(self, idx):
        ...
        if self.mode=='train':
            if len(datum_info_idx) == 5:
                ...
                if task_template_range=='1-1':
                    ...
                    #t_set=['1-3-1-1','1-3-1-2','1-3-3-1','1-3-3-2','1-3-3-3','1-3-3-4'] 
                    task_template=self.all_tasks[task_name][t_set[random.randint(0,len(t_set)-1)]]
        ...
        if task_name == 'link':
            ...
            if task_template['id'] == '5-5-5-5':   
                ...
                abss=self.node_feature[point][1] 
                while len(self.tokenizer.encode(abss))>450:
                    abss=self.tokenizer.decode(self.tokenizer.encode(abss)[:449],skip_special_tokens=True)
                source_text =task_template['source'].format('<extra_id_0>', abss, '<extra_id_0>')
                real_id=[self.re_id[point],self.re_id[point]] 
                target_text = task_template['target'].format(label)
