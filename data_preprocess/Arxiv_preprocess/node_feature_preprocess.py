...
# Load pickle file
raw = load_pickle('full_Llama_node_feature.pkl')
...
for i in tqdm(range(len(raw))):
    ...
    if final_tensor is None:
        final_tensor = pooler_output.detach().clone().to(device)
    else:
        final_tensor = torch.cat((final_tensor,pooler_output),dim=0).to(device)

final_tensor = final_tensor.to("cpu")        
save_pickle(final_tensor, 'final_arxiv_node_feature_BERT.pkl')
