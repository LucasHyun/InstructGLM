...
class Trainer(TrainerBase):
    def __init__(self, args, train_loader=None, val_loader=None, test_loader=None, train=True,val_list=None):
        ...
        self.model.resize_token_embeddings(self.tokenizer.vocab_size+2708)   # 2708 nodes in Cora graph
        ...
        #ckpt_path="restart_checkpoint_name.pth"
        ...
        for epoch in range(self.args.epoch):
            ...
            for step_i, batch in enumerate(self.train_loader):  
                ...
                if global_step==len(self.train_loader)//2:
                    if self.verbose:
                        torch.save(self.model.state_dict(),"flan_cora_{}_{}_8_{}_mid2.pth".format(epoch+1,self.args.lr,self.args.train))
                if global_step==len(self.train_loader)//4:
                    if self.verbose:
                        torch.save(self.model.state_dict(),"flan_cora_{}_{}_8_{}_mid1.pth".format(epoch+1,self.args.lr,self.args.train))
                if global_step==len(self.train_loader)*3//4:
                    if self.verbose:
                        torch.save(self.model.state_dict(),"flan_cora_{}_{}_8_{}_mid3.pth".format(epoch+1,self.args.lr,self.args.train))
                if global_step==len(self.train_loader)//8:
                    if self.verbose:
                        torch.save(self.model.state_dict(),"flan_cora_{}_{}_8_{}_mmid1.pth".format(epoch+1,self.args.lr,self.args.train))
                if global_step==len(self.train_loader)*3//8:
                    if self.verbose:
                        torch.save(self.model.state_dict(),"flan_cora_{}_{}_8_{}_mmid2.pth".format(epoch+1,self.args.lr,self.args.train))
                if global_step==len(self.train_loader)*5//8:
                    if self.verbose:
                        torch.save(self.model.state_dict(),"flan_cora_{}_{}_8_{}_mmid3.pth".format(epoch+1,self.args.lr,self.args.train))
                if global_step==len(self.train_loader)*7//8:
                    if self.verbose:
                        torch.save(self.model.state_dict(),"flan_cora_{}_{}_8_{}_mend.pth".format(epoch+1,self.args.lr,self.args.train))
            ...
            if self.verbose:
                torch.save(self.model.state_dict(),"flan_cora_{}_{}_8_{}_end.pth".format(epoch+1,self.args.lr,self.args.train))
    ...
    def test(self):   
        for epoch in range(4*self.args.epoch):  
            if (epoch+1)%4==1:
                ckpt_path = "flan_cora_{}_{}_8_{}_mid1.pth".format(epoch//4+1,self.args.lr,self.args.train) 
            elif (epoch+1)%4==2:
                ckpt_path = "flan_cora_{}_{}_8_{}_mid2.pth".format(epoch//4+1,self.args.lr,self.args.train) 
            elif (epoch+1)%4==3:
                ckpt_path = "flan_cora_{}_{}_8_{}_mid3.pth".format(epoch//4+1,self.args.lr,self.args.train) 
            elif (epoch+1)%4==0:
                ckpt_path = "flan_cora_{}_{}_8_{}_end.pth".format(epoch//4+1,self.args.lr,self.args.train)

            #ckpt_path='your_test_checkpoint.pth'
