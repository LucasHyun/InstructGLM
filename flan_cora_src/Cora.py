...
class Cora_Dataset(Dataset):
    def __init__(self, all_tasks, task_list, tokenizer, args, sample_numbers, mode='train', split='', rating_augment=False, sample_type='random'): 
        self.all_tasks = all_tasks    #i.e. all instruction prompts
        ...
        self.re_id=load_pickle(os.path.join('Cora','final_cora_re_id.pkl'))  #2
        self.l_max=self.args.max_text_length
        self.real_feature=load_pickle(os.path.join('Cora','final_cora_real_feature.pkl')) #3
        self.train_L1=load_pickle(os.path.join('Cora','final_cora_L1.pkl'))  #4
        self.transductive=load_pickle(os.path.join('Cora','final_cora_transductive.pkl'))  #5 a list
        self.classification=load_pickle(os.path.join('Cora','final_cora_classification.pkl'))  #6
        self.node_feature=load_pickle(os.path.join('Cora','final_cora_node_feature.pkl'))  #7
        ...
        for key in list(self.task_list.keys()):     
            if key == 'link':
                for tems in self.task_list[key]: 
                    if '1-3-1-1' in tems:
                        self.total_length += 2708 * 2  # 2708 nodes in Cora graph
